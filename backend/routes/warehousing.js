...
const getListDate = async (req) => {
    ...
    if (totalRows) {
        ...
        const sql = `SELECT * FROM \`product_warehousing\` ORDER BY warehousing_id  DESC LIMIT ${(page - 1) * perPage
            },${perPage}`;
...
router.delete("/api/:warehousing_id", async (req, res) => {
    ...
    const sql = `DELETE FROM product_warehousing WHERE warehousing_id =${warehousing_id}`;
...
router.get("/edit/:warehousing_id", async (req, res) => {
    ...
    const sql = `SELECT * FROM product_warehousing WHERE warehousing_id =${warehousing_id}`;
    ...
    res.render("warehousing/edit",rows[0]);
});
